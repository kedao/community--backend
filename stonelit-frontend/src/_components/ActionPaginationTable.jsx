...
function ActionPaginationTable ({data, setData, fetchUrl, removeUrl, resource, models, toggleProduct}) {
    ...
    const columns = useMemo(
        () => [
            ...
            {
                cell: row => <>
                    <ActionComponent row={row} onClick={handleAction} className="btn btn-outline grey-400 text-grey-600 border-slate">删除</ActionComponent>
                    <Link to={`/${resource}/edit/${row.id}`} className="btn btn-outline-primary ml-1">编辑</Link>
                </>,
                ignoreRowClick: true,
                allowOverflow: true,
                // button: true,
...
export {ActionPaginationTable};

export default ActionPaginationTable;
